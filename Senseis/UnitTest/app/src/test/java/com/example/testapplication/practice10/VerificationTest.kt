...
class VerificationTest {
    ...
    fun `should verify calls`() {
        ...
        verify(exactly = 4) { car.drive(any(), any()) }
        verify(exactly = 0) { car.drive(Direction.SOUTH) }
        verify(atLeast = 1) { car.drive(or(Direction.EAST, Direction.SOUTH), any()) }
        verify(atMost = 3) { car.drive(any(), more(0)) }
        verify(exactly = 1) { car.drive(any()) }
    }
